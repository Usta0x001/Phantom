# REPORT 1 ΓÇö Architecture & How the System Works
...
| deep      | 300            | 15         | high            | on      |
| stealth   | 200            | ΓÇö          | medium          | off     |
| api_only  | 100            | ΓÇö          | medium          | off     |
...
3. **Injects phase-gate messages** at 33%, 66%, and 90% of `max_iterations` to
   force the agent to transition from recon ΓåÆ exploit ΓåÆ report.
...
5. Calls `self.llm.generate(conversation_history)` ΓåÆ gets a streaming response.
6. Adds the response to `state.messages` as an assistant turn.
7. Parses XML tool invocations from the response.
8. Calls `_execute_actions(actions)` ΓåÆ runs tools ΓåÆ appends tool results as user
   turns in `state.messages`.
9. If a `finish_scan` / `agent_finish` tool fires, marks the agent done and
   exits.
10. Stall detection: 3 consecutive no-action iterations ΓåÆ corrective prompt
    injected. 8 consecutive ΓåÆ non-interactive run aborted.
...
1. Prepends the `system_prompt` (built once at startup ΓÇö see ┬º5).
2. Appends an `<agent_identity>` user message (agent name/ID metadata).
3. **Runs `MemoryCompressor.compress_history`** (see ┬º4) ΓÇö this may call the
...
mode (deep ΓåÆ standard ΓåÆ quick), which changes how many skill files are loaded
...
**Compression threshold** = min(120,000, context_window ├ù 0.6). For a 128K
...
model as the agent ΓÇö meaning compression calls cost money at full model rates
...
- Example: a SQL injection sub-agent loads `sql_injection.md` (~several KB) +
  `deep.md` (~3 KB). Its system prompt may be 15,000ΓÇô25,000 tokens on its own.
...
- `terminal_execute` ΓÇö shell commands in the Docker sandbox (nmap, nuclei, sqlmap, ffuf, etc.)
- `python_execute` ΓÇö Python code execution in the sandbox
- `browser_*` ΓÇö browser automation (Playwright/Chromium)
- `proxy_*` ΓÇö Caido proxy traffic inspection
- `create_agent`, `view_agent_graph`, `send_message_to_agent`, `wait_for_message`,
  `agent_finish` ΓÇö multi-agent orchestration
- `create_vulnerability_report` ΓÇö structured vulnerability reporting
- `notes_*`, `file_edit_*`, `web_search`, `think`, `todo_*` ΓÇö auxiliary tools
- `finish_scan` ΓÇö root agent termination
...
5. The parent agent does not block ΓÇö it continues its own loop immediately after
...
compression (the compressor never sees it ΓÇö it lives on the agent object, not
in the message list).

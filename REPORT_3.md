# REPORT 3 ΓÇö Cost Analysis & Token Flow

## The Core Problem

LLM cost = (input tokens per call) ├ù (price per input token) ├ù (number of calls).
...
```
[system message]     ΓåÉ system_prompt (~10kΓÇô40k tokens, fixed per agent)
[user message]       ΓåÉ agent_identity block (~30 tokens)
[user/assistant ...]  ΓåÉ compressed conversation history (variable, can be 50k+)
[user message]       ΓåÉ "Continue the task." sentinel if last msg was assistant
```
...
|---|---|
| Behavioral rules (lines 1ΓÇô165) | ~2,500 |
| Vulnerability priorities (lines 167ΓÇô205) | ~500 |
| Multi-agent rules (lines 207ΓÇô328) | ~1,200 |
| Tool call format + tool schema | ~2,000ΓÇô4,000 |
| Environment description (lines 391ΓÇô448) | ~600 |
| Scan mode skill (e.g. deep.md ~157 lines) | ~1,500 |
| Per-vulnerability skill (e.g. sql_injection.md) | ~3,000ΓÇô6,000 each |

A root agent in deep mode with no extra skills = ~9,000ΓÇô12,000 tokens.
A SQL injection sub-agent loading `sql_injection.md` + `deep.md` = ~15,000ΓÇô20,000 tokens.
An agent loading 3 skill files = ~22,000ΓÇô28,000 tokens.
...
**After 1 iteration**:
- +1 assistant message (LLM response with thinking + tool call): ~200ΓÇô800 tokens
- +1 user message (tool result): depends on the tool

**Tool output sizes (typical)**:
| Tool | Typical output size |
|---|---|
| `nmap -sV -p 1-1000 target` | 2,000ΓÇô8,000 tokens |
| `nuclei -target url` | 3,000ΓÇô15,000 tokens |
| `sqlmap --level=3` | 4,000ΓÇô12,000 tokens |
| `ffuf -w wordlist` | 5,000ΓÇô20,000 tokens |
| `katana -u target` | 3,000ΓÇô10,000 tokens |
| `subfinder -d target` | 500ΓÇô3,000 tokens |
| Python script output | 200ΓÇô10,000 tokens |

**After 10 iterations with common recon tools**, the history is already
20,000ΓÇô80,000 tokens. The compressor fires at 76,800 tokens (60% of 128K
default). Once it fires, the history shrinks ΓÇö but it grows again quickly
...
- 20 compression calls ├ù 4,000 input tokens = 80,000 input tokens = **$0.24**
- 20 compression calls ├ù 1,500 output tokens = 30,000 output tokens = **$0.45**
...
- 15 ├ù 40,000 = 600,000 tokens of inherited context sent in first-iteration
  calls alone.
...
~500ΓÇô2,000 tokens to the parent's history permanently.

With 15 sub-agents each completing and reporting: +7,500ΓÇô30,000 tokens in the
parent's history.
...
Output: ~300ΓÇô1,000 tokens (a tool call with reasoning).

At $3/M in + $15/M out (claude-3-5-sonnet):
- Input: 47,430 ├ù $3/M = **$0.14**
- Output: 600 ├ù $15/M = **$0.009**
- **Per iteration: ~$0.15**
...
**Estimated total for one deep scan on a medium target: $100ΓÇô$200** using
...
### #1 ΓÇö Untruncated tool outputs stored in history
**Impact: VERY HIGH**
...
### #2 ΓÇö System prompt sent every iteration
**Impact: HIGH**

A 20,000-token system prompt ├ù 300 iterations = 6,000,000 input tokens just
...
models ΓÇö `enable_prompt_caching=True`). Reduce skill file sizes.

### #3 ΓÇö Sub-agent context inheritance by default
**Impact: HIGH**

`inherit_context=True` by default means every child starts with the full parent
history. Most child agents don't need the full parent history ΓÇö they need a
...
### #4 ΓÇö Compression using the same expensive model
**Impact: MEDIUM-HIGH**
...
### #5 ΓÇö Too many agents running simultaneously
**Impact: MEDIUM**
...
iteration 30ΓÇô60 of a deep scan ΓÇö which aligns exactly with the math above.

At iteration 50, the root agent has already spent ~$7ΓÇô$15 just in LLM calls,
and the sub-agents it spawned (5ΓÇô10 of them) have each spent another $2ΓÇô$8.
Total at iteration 50 across all agents: **$20ΓÇô$50** before finding anything
deeply interesting.
...
- **Input tokens: ~92ΓÇô95%** of all tokens
- **Output tokens: ~5ΓÇô8%** of all tokens

This is important: output tokens cost 5├ù more per token on most models, but
...
|---|---|---|
| Set `PHANTOM_COMPRESSOR_LLM=gpt-4o-mini` | 30ΓÇô60% of compression costs | 2 minutes |
| Enable Anthropic prompt caching | 30ΓÇô50% input cost on Anthropic models | 5 minutes |
| Truncate tool outputs to 2,000 tokens before storing | 40ΓÇô60% history growth reduction | 1 hour |
| Change sub-agent `inherit_context` default to `False` | 20ΓÇô40% first-call input reduction | 30 minutes |

...
phantom offers three scan modes to balance speed and thoroughness.

## Quick

```bash
phantom --target ./app --scan-mode quick
```
...
```bash
phantom --target ./app --scan-mode standard
```
...
```bash
phantom --target ./app --scan-mode deep
```

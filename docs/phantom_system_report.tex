...
\begin{tikzpicture}[
  ...
  decide/.style={diamond, draw=highorg, fill=orange!8, text width=2cm,
    align=center, font=\small, inner sep=1pt},
  arr/.style={-{Stealth[length=2.5mm]}, thick, phantomblue},
  node distance=0.9cm,
]
\node[pstep] (start) {Increment Iteration};
\node[decide, below=of start] (check) {Should\\stop?};
\node[pstep, below=of check] (llm) {LLM.generate()\\(streaming)};
\node[pstep, below=of llm] (parse) {Parse tool calls\\from XML output};
\node[pstep, below=of parse] (exec) {Execute tools\\(sandbox / local)};
\node[pstep, below=of exec] (record) {Auto-record findings\\to ledger};
\node[decide, below=of record] (compress) {Memory\\threshold?};
\node[pstep, below=of compress] (comp) {Compress history\\(LLM summary)};

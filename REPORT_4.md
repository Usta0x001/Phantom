# REPORT 4 ΓÇö Code-Verified Deep Dive & Corrections to Previous Reports
...
| `nuclei` | 5,000 chars | ΓÇö |
| `sqlmap` | 5,000 chars | ΓÇö |
| `ffuf` | 3,000 chars | ΓÇö |
| `nmap` | 3,000 chars | ΓÇö |
| All others | 2,000 chars | ΓÇö |
...
REPORT_3 described the adaptive scan downgrade feature (deep ΓåÆ standard ΓåÆ
...
history ΓÇö which is the largest and most expensive part of each request ΓÇö is
**not cached**. Cache hits only save the ~10kΓÇô40k token system prompt, not the
50k+ history.
...
## 2. Agent Loop ΓÇö Verified Architecture
...
Agents move through phases: `recon` ΓåÆ `scanning` ΓåÆ `exploitation` ΓåÆ `reporting`.
...
Phase minimums are configurable but default to low values (2ΓÇô3 iterations per
...
## 3. Memory Compression ΓÇö Verified Details
...
worth of messages ΓÇö or when the raw message count hits 20, whichever comes
first.
...
**Cost of compression itself**: Each compression call sends ~5kΓÇô15k tokens
(the chunk being compressed) and receives ~500ΓÇô2000 tokens (the summary). At
claude-3-7-sonnet pricing (~$3/$15 per M tokens in/out), a compression call
costs roughly $0.02ΓÇô$0.05. A typical 200-iteration scan compresses 10ΓÇô15 times
per agent, adding $0.20ΓÇô$0.75 per agent just for compression overhead.
...
## 4. Tool Execution ΓÇö Verified Details
...
share tool results ΓÇö if two agents both run the same nmap scan, the scan runs
...
This metadata is included in the message stored to history, adding ~100ΓÇô200
chars of overhead per tool call. Over 200 tool calls this is ~20kΓÇô40k chars of
pure metadata injected into history. Not a major cost driver but not zero.

---

## 5. Agent Spawning & Context Inheritance ΓÇö Verified Details
...
```
Parent:   200 iters ├ù 100k avg input = 20M tokens
Children: 5 ├ù 50 iters ├ù 130k avg input = 32.5M tokens
...
of each child's work, typically 500ΓÇô2000 tokens each. Ten children finishing
at the same time injects 5kΓÇô20k tokens into the parent's history in a single
step ΓÇö a sudden spike that may itself trigger compression.
...
## 6. Reporting Pipeline ΓÇö Verified Details
...
[*] Extracting...
```
The word "error" appears ΓåÆ replay marked as FAILED ΓåÆ finding confidence
downgraded ΓåÆ potentially dropped from report.
...
## 7. System Prompt ΓÇö Active Cost Harm
...
is being told it should run 4├ù longer than it's allowed to. The practical
...
## 8. Skills System ΓÇö What It Actually Does
...
10 targeted ones. The current skills are comprehensive but they are generic ΓÇö
...
| Item | Reality |
|---|---|
| Tool output truncation | EXISTS ΓÇö head+tail, middle discarded |
...
| Compression runs | Every ~10 turns, each call costs $0.02ΓÇô$0.05 |

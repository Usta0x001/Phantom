...
def browser_action(
    ...
    try:
        action_name = _resolve_action_name(action, coordinate, selector)
        ...
        if action_name in navigation_actions:
            return _handle_navigation_actions(manager, action_name, url, tab_id)
        if action_name in interaction_actions:
            return _handle_interaction_actions(
                manager, action_name, coordinate, text, key, tab_id
            )
        if action_name in tab_actions:
            return _handle_tab_actions(manager, action_name, url, tab_id)
        if action_name in utility_actions:
            return _handle_utility_actions(
                manager, action_name, duration, js_code, file_path, tab_id, clear
            )
        if action_name in selector_actions:
        ...
        _raise_unknown_action(action_name)

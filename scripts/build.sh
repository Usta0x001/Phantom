...
echo -e "${BLUE}🦉 phantom Build Script${NC}"
echo "================================"
...
poetry run pyinstaller phantom.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="PHANTOM-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/phantom.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/phantom.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/phantom" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/phantom" "$RELEASE_DIR/$BINARY_NAME"

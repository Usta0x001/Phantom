# REPORT 5 ΓÇö Final Honest Verdict
...
| Authenticated scanning | 1/10 | No auth management ΓÇö most real apps are inaccessible |
| Cost efficiency | 2/10 | $50ΓÇô$200 per scan is not viable for most use cases |
...
| Speed | 3/10 | 30ΓÇô90 minutes for a typical deep scan |
...
Estimate: **60ΓÇô80% of real enterprise applications** require authentication for
their meaningful attack surface. Phantom cannot reach this surface.

### 3.2 Anything Requiring State
Vulnerabilities that require multiple steps in a specific order (add item ΓåÆ
checkout ΓåÆ manipulate price ΓåÆ confirm) require the agent to plan a stateful
...
```
Iterations 1ΓÇô50:   50 ├ù $0.164 = $8.20
Iterations 51ΓÇô150: 100 ├ù $0.240 = $24.00
Iterations 151ΓÇô200: 50 ├ù $0.316 = $15.80
Compression overhead: ~10 calls ├ù $0.04 = $0.40
Total root agent: ~$48.40
```

Each child agent (inherit_context=True, starting at 80k tokens):
```
50 iterations ├ù $0.280 avg = $14.00
Compression overhead: ~4 calls ├ù $0.04 = $0.16
Total per child: ~$14.16
```

**Typical deep scan (1 root + 5 children)**: ~$48 + 5├ù$14 = **~$118**

**With additional validation agents**: add ~$5ΓÇô10 each ΓåÆ **$130ΓÇô$160 total**

### 4.3 Output per Dollar

Assuming the scan finds 5ΓÇô10 real vulnerabilities:
- Cost per verified finding: **$13ΓÇô$32**

A professional penetration tester charges $150ΓÇô$300/hour and finds 10ΓÇô20
vulnerabilities in an 8-hour engagement ($60ΓÇô$240/finding). Phantom's cost per
finding is competitive only if the finding quality is similar ΓÇö and for
...
|---|---|---|---|---|
| **Phantom (deep)** | $100ΓÇô$160 | None | Occasional | 30ΓÇô90 min |
| **Nuclei** | ~$0 (compute) | None | No | 2ΓÇô5 min |
| **Burp Suite Pro** | ~$0 (compute) | Yes | No | 10ΓÇô30 min |
| **ZAP** | ~$0 (compute) | Yes | No | 10ΓÇô30 min |
| **Pentest GPT** | ~$5ΓÇô20 | Guided | Yes (guided) | N/A (manual) |
| **Human pentester** | $1,200ΓÇô$2,400/day | Yes | Yes | 1ΓÇô3 days |
...
- Agent-generated findings (no tool backing): **~40ΓÇô60% true positive rate**
  (LLM may hallucinate vulnerability existence)
- PoC-backed findings: **~70% true positive rate** (PoC replay verification
  is broken ΓÇö see REPORT_4 ┬º6.3 ΓÇö so "PoC verified" label is unreliable)
...
Expected bias: actual CVSS 6.5 findings reported as 7.5ΓÇô8.5 approximately
30ΓÇô40% of the time.

### 6.3 Reproducibility

Same target, same config, different run ΓåÆ different finding set. Reasons:
...
Estimated overlap between two runs of the same scan: **50ΓÇô70%** of findings
appear in both runs. 30ΓÇô50% are run-specific.

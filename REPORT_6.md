...
Priority tiers:
- **P0** ΓÇö Fix now. High impact, low effort, no architectural change needed.
- **P1** ΓÇö Fix soon. High impact, moderate effort.
- **P2** ΓÇö Fix when time allows. Moderate impact, higher effort.
- **P3** ΓÇö Future work. High impact but requires architectural change.

---

## P0 ΓÇö Immediate Wins (Low Effort, High Impact)

### P0.1 ΓÇö Remove / Replace the "2000+ steps minimum" Instruction
...
**Expected impact**: 15ΓÇô25% reduction in iteration count. Rough cost saving:
$15ΓÇô$40 per scan.
...
### P0.2 ΓÇö Enable Adaptive Scan Mode by Default
...
```python
phantom_adaptive_scan_threshold_1 = "30"   # % of budget ΓåÆ downgrade deepΓåÆstandard
phantom_adaptive_scan_threshold_2 = "60"   # % of budget ΓåÆ downgrade standardΓåÆquick
```
...
mode can cut the final 30ΓÇô40% of scan cost in half.

**Effort**: 2 minutes (config change) + verify threshold values are reasonable.

---

### P0.3 ΓÇö Enable Model Routing by Default
...
make up roughly 40ΓÇô60% of all iterations.

**Expected impact**: 25ΓÇô35% reduction in LLM cost (tool-execution turns at
...
### P0.4 ΓÇö Fix PoC Replay Success Detection
...
**Effort**: 2ΓÇô3 hours (write SUCCESS_PATTERNS dict, update detection logic,
test against known good PoC outputs).

---

### P0.5 ΓÇö Disable Context Inheritance for ALL Sub-Agents by Default
...
The parent agent should generate a brief context summary (1ΓÇô3 paragraphs: what
...
reduce child agent initial token load by 85ΓÇô95%.

**Expected impact**: For a scan with 5 child agents: save ~5 ├ù 80k ├ù 50 iters
= 20M tokens = ~$60 per scan.

**Effort**: 4ΓÇô6 hours (change default, update callers to pass context_summary,
update system prompt to instruct root agent to write summaries when spawning).

---

## P1 ΓÇö High-Impact Improvements (Moderate Effort)

### P1.1 ΓÇö Fix Tool Truncation: Middle-Preserving Strategy for ffuf and nmap
...
**Effort**: 3ΓÇô4 hours (implement per-tool extractors, test against real tool
output samples).

---

### P1.2 ΓÇö Implement a Shared Finding Registry
...
async def should_scan(endpoint: str, tool: str, scan_registry: ScanRegistry) -> bool:
    if await scan_registry.was_scanned(endpoint, tool):
        return False  # Skip ΓÇö already done
...
**Effort**: 6ΓÇô8 hours (design registry schema, integrate with tool execution
pipeline, handle partial/failed scans, test with concurrent agents).

---

### P1.3 ΓÇö Structured Finding Extraction vs Compression
...
**Effort**: 8ΓÇô12 hours (anchor extraction heuristics, state schema change,
injection logic, testing).

---

### P1.4 ΓÇö Add Basic Session/Cookie Management

**Problem**: No authentication support means 60ΓÇô80% of real application attack
...
**Effort**: 12ΓÇô20 hours (login tool, session store, cookie injection for curl/
ffuf/nuclei/sqlmap, session refresh detection).

---

## P2 ΓÇö Moderate-Impact, Higher-Effort Improvements

### P2.1 ΓÇö Extend History Caching Beyond System Prompt
...
The first ~50 messages of a 200-iteration scan are the same across calls 51ΓÇô200.
...
40ΓÇô60% of input token cost for mid-to-late iterations. Anthropic charges 10%
of normal price for cache hits.

**Effort**: 4ΓÇô6 hours (implement and verify cache control placement, test that
cache hit rate is actually high).

---

### P2.2 ΓÇö Replace LLM-based CVSS Scoring with Formula-Based Scoring
...
def compute_cvss31(av, ac, pr, ui, s, c, i, a) -> float:
    """Standard CVSS 3.1 formula ΓÇö not LLM-generated."""
...
**Effort**: 3ΓÇô4 hours (implement formula, update LLM prompt to produce
structured vector components, validate against known CVSS scores).

---

### P2.3 ΓÇö Smarter Stall Detection: Semantic Stall
...
**Effort**: 6ΓÇô8 hours (define progress metrics, track them in agent state,
implement detection, write intervention prompts).

---

### P2.4 ΓÇö Per-Agent Cost Budget
...
**Effort**: 6ΓÇô8 hours (state change, enforcement in LLM layer, budget
allocation logic in create_agent, handling of budget exhaustion).

---

## P3 ΓÇö Architectural Changes (Future Work)

### P3.1 ΓÇö Dynamic Payload Generation
...
**Estimated effort**: 40ΓÇô80 hours (new component, LLM-assisted payload mutation,
feedback loop from tool results).

### P3.2 ΓÇö Response Diffing Engine
...
**Estimated effort**: 20ΓÇô30 hours (diff engine, integration with terminal tool,
prompt updates to use diff results).

### P3.3 ΓÇö Persistent Cross-Scan Knowledge Base
...
**Estimated effort**: 30ΓÇô50 hours (knowledge store, similarity matching,
...
|---|---|---|---|
| P0 (all 5) | 5 quick fixes | **40ΓÇô55% cost reduction** | **+25% quality** |
| P1 (all 4) | 4 medium items | **+15ΓÇô20% additional** | **+40% quality** |
| P2 (all 4) | 4 larger items | **+10ΓÇô15% additional** | **+20% quality** |
| **P0+P1 combined** | 9 items | **~55ΓÇô75% total cost reduction** | **~65% quality gain** |

After P0+P1 implementation:
- Expected scan cost: $30ΓÇô$60 (down from $100ΓÇô$160)
...
- Unlocks 60ΓÇô80% of real application attack surfaces
...
1. **Week 1**: P0.1, P0.2, P0.3 (config + prompt changes ΓÇö 1 hour total)
2. **Week 1**: P0.4 (fix PoC replay ΓÇö 3 hours)
3. **Week 2**: P0.5 (context inheritance default ΓÇö 6 hours)
4. **Week 2**: P1.1 (fix ffuf/nmap truncation ΓÇö 4 hours)
5. **Week 3**: P1.2 (shared finding registry ΓÇö 8 hours)
6. **Week 4**: P1.3 (finding anchors ΓÇö 12 hours)
7. **Month 2**: P1.4 (session management ΓÇö 20 hours)
